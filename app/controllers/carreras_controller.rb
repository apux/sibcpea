class CarrerasController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @carrera.save
        format.html { redirect_to(@carrera, :notice => 'Carrera fue creada satisfactoriamente.') }
  ...
  def update
    ...
    respond_to do |format|
      if @carrera.update_attributes(params[:carrera])
        format.html { redirect_to(@carrera, :notice => 'Carrera fue modificada exitosamente.') }
        format.xml  { head :ok }
      else
